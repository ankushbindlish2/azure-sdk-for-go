...
5. If any non-automation commit introduces real logic changes to a `DO NOT EDIT` file, record the file path, short commit SHA, and author. This is a **blocking issue** — it will be included in the Step 6 comment.

### Step 5 — Identify gaps to merge

If the PR is mergeable (`Squash and merge` enabled), skip to Step 6 and comment `## PR is ready to merge`.
...
**If auto-generated files were manually edited** (detected in Step 4), always include this block at the top of the comment, **before** any CI failures:
