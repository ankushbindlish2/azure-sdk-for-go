...
type InformationProtectionPoliciesServer struct {
	...
	CreateOrUpdate func(ctx context.Context, resourceID string, informationProtectionPolicyName armsecurity.InformationProtectionPolicyName, informationProtectionPolicy armsecurity.InformationProtectionPolicy, options *armsecurity.InformationProtectionPoliciesClientCreateOrUpdateOptions) (resp azfake.Responder[armsecurity.InformationProtectionPoliciesClientCreateOrUpdateResponse], errResp azfake.ErrorResponder)

	// Get is the fake for method InformationProtectionPoliciesClient.Get
	// HTTP status codes to indicate success: http.StatusOK
	Get func(ctx context.Context, resourceID string, informationProtectionPolicyName armsecurity.InformationProtectionPolicyName, options *armsecurity.InformationProtectionPoliciesClientGetOptions) (resp azfake.Responder[armsecurity.InformationProtectionPoliciesClientGetResponse], errResp azfake.ErrorResponder)

	// NewListPager is the fake for method InformationProtectionPoliciesClient.NewListPager
	// HTTP status codes to indicate success: http.StatusOK
	NewListPager func(resourceID string, options *armsecurity.InformationProtectionPoliciesClientListOptions) (resp azfake.PagerResponder[armsecurity.InformationProtectionPoliciesClientListResponse])
}
...
func (i *InformationProtectionPoliciesServerTransport) dispatchCreateOrUpdate(req *http.Request) (*http.Response, error) {
	...
	const regexStr = `/(?P<resourceId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Security/informationProtectionPolicies/(?P<informationProtectionPolicyName>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)`
	...
	respr, errRespr := i.srv.CreateOrUpdate(req.Context(), resourceIDParam, informationProtectionPolicyNameParam, body, nil)
...
func (i *InformationProtectionPoliciesServerTransport) dispatchGet(req *http.Request) (*http.Response, error) {
	...
	respr, errRespr := i.srv.Get(req.Context(), resourceIDParam, informationProtectionPolicyNameParam, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
...
func (i *InformationProtectionPoliciesServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/(?P<resourceId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Security/informationProtectionPolicies`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		resp := i.srv.NewListPager(resourceIDParam, nil)
		newListPager = &resp
