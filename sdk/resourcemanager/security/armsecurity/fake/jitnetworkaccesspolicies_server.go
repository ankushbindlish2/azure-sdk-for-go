...
func (j *JitNetworkAccessPoliciesServerTransport) dispatchInitiate(req *http.Request) (*http.Response, error) {
	...
	jitNetworkAccessPolicyInitiateTypeParam, err := parseWithCast(matches[regex.SubexpIndex("jitNetworkAccessPolicyInitiateType")], func(v string) (armsecurity.JitNetworkAccessPolicyInitiateType, error) {
		p, unescapeErr := url.PathUnescape(v)
		if unescapeErr != nil {
			return "", unescapeErr
		}
		return armsecurity.JitNetworkAccessPolicyInitiateType(p), nil
	})
	if err != nil {
		return nil, err
	}
	respr, errRespr := j.srv.Initiate(req.Context(), resourceGroupNameParam, ascLocationParam, jitNetworkAccessPolicyNameParam, jitNetworkAccessPolicyInitiateTypeParam, body, nil)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
	respContent := server.GetResponseContent(respr)
