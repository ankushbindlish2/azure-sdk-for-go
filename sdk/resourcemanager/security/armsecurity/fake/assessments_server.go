...
type AssessmentsServer struct {
	...
	// HTTP status codes to indicate success: http.StatusOK
	NewListPager func(resourceID string, options *armsecurity.AssessmentsClientListOptions) (resp azfake.PagerResponder[armsecurity.AssessmentsClientListResponse])
}
...
func (a *AssessmentsServerTransport) dispatchNewListPager(req *http.Request) (*http.Response, error) {
	...
	if newListPager == nil {
		const regexStr = `/(?P<resourceId>[!#&$-;=?-\[\]_a-zA-Z0-9~%@]+)/providers/Microsoft\.Security/assessments`
		regex := regexp.MustCompile(regexStr)
		matches := regex.FindStringSubmatch(req.URL.EscapedPath())
		if len(matches) < 2 {
			return nil, fmt.Errorf("failed to parse path %s", req.URL.Path)
		...
		resp := a.srv.NewListPager(resourceIDParam, nil)
		newListPager = &resp
