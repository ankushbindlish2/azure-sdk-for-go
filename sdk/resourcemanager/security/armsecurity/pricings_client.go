...
func (client *PricingsClient) Delete(ctx context.Context, resourceID string, pricingName string, options *PricingsClientDeleteOptions) (PricingsClientDeleteResponse, error) {
	var err error
	const operationName = "PricingsClient.Delete"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.deleteCreateRequest(ctx, resourceID, pricingName, options)
	if err != nil {
...
func (client *PricingsClient) deleteCreateRequest(ctx context.Context, resourceID string, pricingName string, _ *PricingsClientDeleteOptions) (*policy.Request, error) {
	urlPath := "/{resourceId}/providers/Microsoft.Security/pricings/{pricingName}"
	if resourceID == "" {
...
func (client *PricingsClient) Get(ctx context.Context, resourceID string, pricingName string, options *PricingsClientGetOptions) (PricingsClientGetResponse, error) {
	var err error
	const operationName = "PricingsClient.Get"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.getCreateRequest(ctx, resourceID, pricingName, options)
	if err != nil {
...
func (client *PricingsClient) getCreateRequest(ctx context.Context, resourceID string, pricingName string, _ *PricingsClientGetOptions) (*policy.Request, error) {
	urlPath := "/{resourceId}/providers/Microsoft.Security/pricings/{pricingName}"
	if resourceID == "" {
		return nil, errors.New("parameter resourceID cannot be empty")
	}
...
// NewListPager - Lists Microsoft Defender for Cloud pricing configurations of the scopeId, that match the optional given
// $filter. Valid scopes are: subscription id or a specific resource id (Supported resources are: 'VirtualMachines, VMSS and
// ARC Machines'). Valid $filter is: 'name in ({planName1},{planName2},...)'. If $filter is not provided, the unfiltered list
// will be returned. If '$filter=name in (planName1,planName2)' is provided, the returned list includes the pricings set for
// 'planName1' and 'planName2' only.
//
// Generated from API version 2024-01-01
//   - resourceID - The fully qualified Azure Resource manager identifier of the resource.
//   - options - PricingsClientListOptions contains the optional parameters for the PricingsClient.NewListPager method.
func (client *PricingsClient) NewListPager(resourceID string, options *PricingsClientListOptions) *runtime.Pager[PricingsClientListResponse] {
	return runtime.NewPager(runtime.PagingHandler[PricingsClientListResponse]{
		More: func(page PricingsClientListResponse) bool {
			return false
		},
		Fetcher: func(ctx context.Context, page *PricingsClientListResponse) (PricingsClientListResponse, error) {
			ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, "PricingsClient.NewListPager")
...
func (client *PricingsClient) listCreateRequest(ctx context.Context, resourceID string, options *PricingsClientListOptions) (*policy.Request, error) {
	urlPath := "/{resourceId}/providers/Microsoft.Security/pricings"
	if resourceID == "" {
		return nil, errors.New("parameter resourceID cannot be empty")
	}
	urlPath = strings.ReplaceAll(urlPath, "{resourceId}", resourceID)
...
func (client *PricingsClient) Update(ctx context.Context, resourceID string, pricingName string, pricing Pricing, options *PricingsClientUpdateOptions) (PricingsClientUpdateResponse, error) {
	var err error
	const operationName = "PricingsClient.Update"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.updateCreateRequest(ctx, resourceID, pricingName, pricing, options)
	if err != nil {
...
func (client *PricingsClient) updateCreateRequest(ctx context.Context, resourceID string, pricingName string, pricing Pricing, _ *PricingsClientUpdateOptions) (*policy.Request, error) {
	urlPath := "/{resourceId}/providers/Microsoft.Security/pricings/{pricingName}"
	if resourceID == "" {
		return nil, errors.New("parameter resourceID cannot be empty")
	}
