...
//     method.
func (client *GallerySharingProfileClient) BeginUpdate(ctx context.Context, apiVersion string, resourceGroupName string, galleryName string, sharingUpdate SharingUpdate, options *GallerySharingProfileClientBeginUpdateOptions) (*runtime.Poller[GallerySharingProfileClientUpdateResponse], error) {
	if options == nil || options.ResumeToken == "" {
		resp, err := client.update(ctx, apiVersion, resourceGroupName, galleryName, sharingUpdate, options)
		if err != nil {
			return nil, err
		}
...
// If the operation fails it returns an *azcore.ResponseError type.
func (client *GallerySharingProfileClient) update(ctx context.Context, apiVersion string, resourceGroupName string, galleryName string, sharingUpdate SharingUpdate, options *GallerySharingProfileClientBeginUpdateOptions) (*http.Response, error) {
	var err error
	const operationName = "GallerySharingProfileClient.BeginUpdate"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.updateCreateRequest(ctx, apiVersion, resourceGroupName, galleryName, sharingUpdate, options)
	if err != nil {
		return nil, err
	}
...
func (client *GallerySharingProfileClient) updateCreateRequest(ctx context.Context, apiVersion string, resourceGroupName string, galleryName string, sharingUpdate SharingUpdate, _ *GallerySharingProfileClientBeginUpdateOptions) (*policy.Request, error) {
