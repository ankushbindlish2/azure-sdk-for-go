...
- All Cloud Services (classic) APIs have been removed following the [retirement of Azure Cloud Services (classic)](https://azure.microsoft.com/en-us/updates?id=486344). Removed APIs include clients `CloudServicesClient`, `CloudServiceRoleInstancesClient`, `CloudServiceRolesClient`, `CloudServiceOperatingSystemsClient`, `CloudServicesUpdateDomainClient`, their factory methods, all associated types (`CloudService*`, `RoleInstance*`, `OSFamily*`, `OSVersion*`, `UpdateDomain*`, `Extension`, `InstanceSKU`, `LoadBalancerConfiguration*`, etc.) and enums (`CloudServiceSlotType`, `CloudServiceUpgradeMode`)
- Type of `OperationListResult.Value` has been changed from `[]*OperationValue` to `[]*Operation`; `OperationValue` has been removed as part of the Cloud Services retirement
