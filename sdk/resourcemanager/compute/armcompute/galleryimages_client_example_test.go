...
func ExampleGalleryImagesClient_BeginCreateOrUpdate() {
	...
	poller, err := clientFactory.NewGalleryImagesClient().BeginCreateOrUpdate(ctx, "2025-03-03", "myResourceGroup", "myGalleryName", "myGalleryImageName", armcompute.GalleryImage{
...
func ExampleGalleryImagesClient_BeginDelete() {
	...
	poller, err := clientFactory.NewGalleryImagesClient().BeginDelete(ctx, "2025-03-03", "myResourceGroup", "myGalleryName", "myGalleryImageName", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	res, err := poller.PollUntilDone(ctx, nil)
...
func ExampleGalleryImagesClient_Get() {
	...
	res, err := clientFactory.NewGalleryImagesClient().Get(ctx, "2025-03-03", "myResourceGroup", "myGalleryName", "myGalleryImageName", nil)
	if err != nil {
		log.Fatalf("failed to finish the request: %v", err)
	}
	// You could use response here. We use blank identifier for just demo purposes.
...
func ExampleGalleryImagesClient_NewListByGalleryPager() {
	...
	pager := clientFactory.NewGalleryImagesClient().NewListByGalleryPager("2025-03-03", "myResourceGroup", "myGalleryName", nil)
	for pager.More() {
...
func ExampleGalleryImagesClient_BeginUpdate_updateASimpleGalleryImage() {
	...
	poller, err := clientFactory.NewGalleryImagesClient().BeginUpdate(ctx, "2025-03-03", "myResourceGroup", "myGalleryName", "myGalleryImageName", armcompute.GalleryImageUpdate{
		Properties: &armcompute.GalleryImageProperties{
			OSType:           to.Ptr(armcompute.OperatingSystemTypesWindows),
			OSState:          to.Ptr(armcompute.OperatingSystemStateTypesGeneralized),
			HyperVGeneration: to.Ptr(armcompute.HyperVGenerationV1),
