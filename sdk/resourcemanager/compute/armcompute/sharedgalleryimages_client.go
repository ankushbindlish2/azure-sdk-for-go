...
func (client *SharedGalleryImagesClient) Get(ctx context.Context, apiVersion string, location string, galleryUniqueName string, galleryImageName string, options *SharedGalleryImagesClientGetOptions) (SharedGalleryImagesClientGetResponse, error) {
	var err error
	const operationName = "SharedGalleryImagesClient.Get"
	ctx = context.WithValue(ctx, runtime.CtxAPINameKey{}, operationName)
	ctx, endSpan := runtime.StartSpan(ctx, operationName, client.internal.Tracer(), nil)
	defer func() { endSpan(err) }()
	req, err := client.getCreateRequest(ctx, apiVersion, location, galleryUniqueName, galleryImageName, options)
	if err != nil {
...
func (client *SharedGalleryImagesClient) getCreateRequest(ctx context.Context, apiVersion string, location string, galleryUniqueName string, galleryImageName string, _ *SharedGalleryImagesClientGetOptions) (*policy.Request, error) {
...
//     method.
func (client *SharedGalleryImagesClient) NewListPager(apiVersion string, location string, galleryUniqueName string, options *SharedGalleryImagesClientListOptions) *runtime.Pager[SharedGalleryImagesClientListResponse] {
	return runtime.NewPager(runtime.PagingHandler[SharedGalleryImagesClientListResponse]{
		...
		Fetcher: func(ctx context.Context, page *SharedGalleryImagesClientListResponse) (SharedGalleryImagesClientListResponse, error) {
			...
			resp, err := runtime.FetcherForNextLink(ctx, client.internal.Pipeline(), nextLink, func(ctx context.Context) (*policy.Request, error) {
				return client.listCreateRequest(ctx, apiVersion, location, galleryUniqueName, options)
			}, nil)
			if err != nil {
...
func (client *SharedGalleryImagesClient) listCreateRequest(ctx context.Context, apiVersion string, location string, galleryUniqueName string, options *SharedGalleryImagesClientListOptions) (*policy.Request, error) {
