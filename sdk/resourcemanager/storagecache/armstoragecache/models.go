...
type AmlFilesystemHsmSettings struct {
	...
	// storage account as the hydration and archive container. The resource provider must have permission to create SAS tokens
	...
	// is only used during initial creation of the AML file system. It automatically creates an import job resource that can be
	// deleted.
	ImportPrefix *string
	...
	// namespace. This is only used during initial creation of the AML file system and has '/' as the default value. It automatically
...
type AutoImportJobProperties struct {
	...
	// a file is at that path, how it handles it. Fail indicates that the auto import job should stop immediately and not do anything
	// with the conflict. Skip indicates that it should pass over the conflict. OverwriteIfDirty causes the auto import job to
	// delete and re-import the file or directory if it is a conflicting type, is dirty, or is currently released. OverwriteAlways
	// extends OverwriteIfDirty to include releasing files that had been restored but were not dirty. Please reference https://learn.microsoft.com/en-us/azure/azure-managed-lustre/blob-integration#conflict-resolution-mode
...
type AutoImportJobPropertiesStatus struct {
	...
	// READ-ONLY; Server-defined status code for auto import job.
	StatusCode *string

	// READ-ONLY; Server-defined status message for auto import job.
	StatusMessage *string
...
type ExpansionJobPropertiesStatus struct {
	...
	// expansion finished successfully. Failed means the expansion was unable to complete due to a fatal error. Deleting indicates
	// the expansion is being rolled back.
...
type ImportJobPropertiesStatus struct {
	...
	// it has been canceled by the user. Completed indicates import finished, successfully importing all discovered blobs into
	// the Lustre namespace. CompletedPartial indicates the import finished but some blobs either were found to be conflicting
	// and could not be imported or other errors were encountered. Failed means the import was unable to complete due to a fatal
	// error.
...
type PrimingJob struct {
	...
	// is in Azure blob storage the URL should include a Shared Access Signature (SAS) granting read permissions on the blob.
...
type Restriction struct {
	...
	// the SKU has requiredQuotas parameter as the subscription does not belong to that quota. "NotAvailableForSubscription" is
	// related to capacity at the datacenter.
